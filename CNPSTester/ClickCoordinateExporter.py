...
def onleftclick(event):
    # wait = lastTime - time.time()
    print("Common.move_mouse_and_left_click", event.Position, " wait: ", time.time())
    lastTime = time.time()
    return True


def onrightclick(event):
    print("Common.move_mouse_and_right_click", event.Position, " wait: ", time.time())
    return True
